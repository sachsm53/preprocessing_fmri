	...
	for subject in subjects:
		for run in runs:
			...
			for c in conditions:
				...
				# if not os.path.exists(outratefile):
				# 	emo = pd.read_csv(ratefile, sep = "\t", header = None)
				# 	emo.columns = ["time", "rating"]
				# 	rating = emo['rating'].divide(12.7,1)
				# 	time = emo.time
				# 	interp = CubicSpline(time,rating)

				# 	#interp = interp1d(time, rating, kind='cubic')
				# 	#qp = np.linspace(0.1,rate_time[songrun],0.1)
				# 	qp = np.linspace(0.1,rate_time[songrun],(rate_time[songrun]*10),endpoint = False)
				# 	interp_values = interp(qp)
				# 	interp_values_noneg = interp_values
				# 	for i,v in enumerate(interp_values):
				# 		if v < 0: 
				# 			#print i,v
				# 			interp_values_noneg[i] = 0


				# 	#Downsample to 10hz
				# 	rate_ds = resample(interp_values_noneg, 515) 
				# 	np.savetxt(outratefile, rate_ds,fmt='%.4f')
