...
for subject in subjects:
	...
	if not args.nomusic:
		...
		# if not args.noconfound:
		# 	filteredfile = "%s/%s" % (runfolder,datafiles[run])
		# 	command = "fsl_glm -i %s --demean -m %s/mask.nii.gz -d %s --out_res=%s/filtered_func_data_residuals.nii.gz" % (filteredfile,runfolder,designfile,runfolder)
		# 	call(command,shell=True)
		# 	print
