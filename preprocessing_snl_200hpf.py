...
parser.add_argument("--noslicetime",help="skip creating slicetime files", action="store_true")
...
def dolowerlevels(designsuffix):
	for run in runs:
		...
		if (designsuffix):
			genericfile = designpath + "/" + designsuffix + ".fsf"
			outputfile = funcfolder + 'design_music_sadnl_200hpf.fsf'
...
def dotf(task):
	#THIS IS FOR Low-pass filtering (temporal smoothing at 5s) WHICH IS NOT ALWAYS WHAT YOU WANT TO DO
	for run in runs:
		song = songs[run-1]
		timepoints = song_timepoints[run-1]
		run = str(run)
		...
		tffile = "%sfiltered_func_data_lpf.nii.gz" % (featfile)

		print sectionColor + "-----Temporal smoothing for %s%s" %(song,mainColor)

		if not os.path.exists(tffile):
			hp_sigma = -1 #10 seconds in TRs, pass signal faster than .01 Hz
			lp_sigma = 62.5

			command = "fslmaths %s -bptf %d %d %s -odt float" % (inputfile,hp_sigma,lp_sigma,tffile)
			print yellow + command + mainColor
			call(command,shell=True)

			command = "fslmeants -i %s -c 32 32 20 -o %sorigdata_samplets.txt" % (inputfile,featfile)
			call(command,shell=True)
			command = "fslmeants -i %s -c 32 32 20 -o %sfiltdata_samplets.txt" % (tffile,featfile)
			call(command,shell=True)

			command="fsl_tsplot -i %sfiltdata_samplets.txt -o %sfilt.png -t 'Bandpass Filtered Data'" % (featfile,featfile)
			call(command,shell=True)
			command="fsl_tsplot -i %sorigdata_samplets.txt -o %sorig.png -t 'Unfiltered Data'" % (featfile,featfile)
			call(command,shell=True)
			#writeToLog("<h2>Temporal Filtering</h2><br><img src=orig.png><br><br><img src=filt.png><br><br><hr>",reportfile)

		else: 
			print sectionColor2 + "Temporal filtering already completed for %s. Moving on\n%s"  % (song,mainColor)
...
def doconfound(task):
	...
	for run in runs:
		...
		inputfile = "%sfiltered_func_data.nii.gz" % (featfile) #This one has already been temporally smoothed at 128
...
for subject in subjects:
	...
	if not args.nomusic:
		...
		if not args.nofeat:
			...
			dolowerlevels("generic_pre_sadln_200hpf") #no registration, copy from resting state
